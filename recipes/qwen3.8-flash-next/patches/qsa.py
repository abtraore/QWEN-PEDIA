...
class Qwen3_8FlashNextQSAFlashAttentionBackend(FlashAttentionBackend):
    ...
    # with the layer's per-tensor scales and dequantized on load inside the
    # QSA Triton kernel. flash-attn never touches the sparse attention math,
    # so its fp8 capability probe does not apply here (see the override).
...
class Qwen3_8FlashNextQSAFlashAttentionImpl(FlashAttentionImpl):
    ...
    def __init__(self, *args, **kwargs) -> None:
        # The parent constructor probes flash-attn for quantized-KV support
        # and raises on sm120 -- but QSA's sparse Triton kernel does its own
        # fp8 dequant on load and never runs flash-attn over the cache. Hand
        # the parent "auto" and restore the real dtype afterwards (the parent
        # uses it only for that probe, an sm90-only FA4 flag, and storage).
        args = list(args)
        _real_kv_dtype = None
        if kwargs.get("kv_cache_dtype") in ("fp8", "fp8_e4m3"):
            _real_kv_dtype = kwargs["kv_cache_dtype"]
            kwargs["kv_cache_dtype"] = "auto"
        elif len(args) > 6 and args[6] in ("fp8", "fp8_e4m3"):
    ...
    def forward_qsa(
        ...
        if self.kv_cache_dtype in ("fp8", "fp8_e4m3"):
            ...
            # Same-itemsize views keep shape and strides.
    ...
    def __init__(
        ...
        *,
        ...
        if self.kv_cache_torch_dtype not in (torch.bfloat16, torch.uint8):
            raise NotImplementedError(
                "Qwen3.8-Flash-Next QSA requires BF16 or FP8-e4m3 cache storage"
            )
