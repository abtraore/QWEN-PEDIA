...
class Qwen3_8FlashNextNGramEmbedding(PleOffloadLayer):
    def __init__(
        ...
        # A 62 GiB host cannot hold the ~51 GB table as anonymous memory next
        # to the engine processes, so the offload process may serve it from a
        # read-only memory map instead: pages are clean and evictable, and the
        # boot-time RSS spike (which starved NVRM of system memory) disappears.
        self._mmap_table = False
        mmap_path = os.environ.get("VLLM_PLE_MMAP_FILE", "")
        if mmap_path and is_offload_process():
            self._attach_mmap_table(mmap_path)

    def _attach_mmap_table(self, path: str) -> None:
        import numpy as np
        ...
        if weight.dtype != torch.float8_e4m3fn:
            raise ValueError(
                f"VLLM_PLE_MMAP_FILE requires an fp8 table, got {weight.dtype}"
            )
        expected = rows * head_dim * weight.element_size()
        actual = os.path.getsize(path)
        if actual != expected:
            raise ValueError(
                f"PLE mmap file {path} holds {actual} bytes, expected "
                f"{expected} for a {rows}x{head_dim} fp8 table"
            )
        # mode="c" (copy-on-write) keeps the file immutable while letting any
        # stray in-place write land in private pages instead of failing.
        mapped = np.memmap(path, dtype=np.uint8, mode="c", shape=(rows, head_dim))
        weight.data = torch.from_numpy(mapped).view(torch.float8_e4m3fn)
        self._mmap_table = True
        ...
        for name, loaded_weight in weights:
            ...
            if name.startswith(shard_prefix) and name.endswith(".weight"):
                ...
                if not shard_text.isdigit():
                    ...
                    continue
                if self._mmap_table:
                    # The table is served from VLLM_PLE_MMAP_FILE. Leaving the
                    # shard tensors untouched keeps their mmap-backed pages
                    # unread, so boot skips ~51 GB of checkpoint I/O.
