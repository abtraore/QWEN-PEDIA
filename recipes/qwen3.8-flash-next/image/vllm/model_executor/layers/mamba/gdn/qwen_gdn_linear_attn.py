...
class QwenGatedDeltaNetAttention(GatedDeltaNetAttention):
    ...
    def __init__(
        ...
        # saturn (2026-09-05): optional fused [qkvz ; ba] input projection built
        # after weight loading (see fuse_in_proj); one GEMM per layer instead of
        # two at decode. Set by the model's load_weights when
        # VLLM_GDN_FUSE_IN_PROJ=1.
        self._fused_in_proj_weight: torch.Tensor | None = None
        self._fused_in_proj_sizes: tuple[int, int] | None = None
    ...
    def fuse_in_proj(self) -> bool:
        """Concatenate in_proj_qkvz and in_proj_ba into one bf16 weight.

        Both are MergedColumnParallelLinear with the same per-rank head group,
        so the per-rank row concatenation is exact. The original parameters
        are re-pointed at views of the fused buffer (no extra memory, the
        modules keep working for any non-fused path). Only for unquantized
        bf16 weights without bias; returns False when not applicable.
        """
        qkvz = getattr(self, "in_proj_qkvz", None)
        ba = getattr(self, "in_proj_ba", None)
        if qkvz is None or ba is None or self.disable_tp_for_ba_proj:
            self._fuse_in_proj_reason = "missing proj or replicated ba"
            return False
        wq, wb = getattr(qkvz, "weight", None), getattr(ba, "weight", None)
        if wq is None or wb is None or wq.dim() != 2 or wb.dim() != 2:
            self._fuse_in_proj_reason = f"weight shape {getattr(wq, 'shape', None)} {getattr(wb, 'shape', None)}"
            return False
        if wq.dtype != torch.bfloat16 or wb.dtype != torch.bfloat16:
            self._fuse_in_proj_reason = f"dtype {wq.dtype} {wb.dtype}"
            return False
        if getattr(qkvz, "bias", None) is not None or getattr(ba, "bias", None) is not None:
            self._fuse_in_proj_reason = "bias present"
            return False
        if wq.shape[1] != wb.shape[1]:
            self._fuse_in_proj_reason = f"K mismatch {wq.shape} {wb.shape}"
            return False
        fused = torch.cat([wq.data, wb.data], dim=0).contiguous()
        wq.data = fused[: wq.shape[0]]
        wb.data = fused[wq.shape[0] :]
        self._fused_in_proj_weight = fused
        self._fused_in_proj_sizes = (wq.shape[0], wb.shape[0])
        return True

    def _project_in(self, hidden_states: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """(mixed_qkvz, ba) from one fused GEMM when available, else two."""
        if self._fused_in_proj_weight is not None and hidden_states.dtype == torch.bfloat16:
            w = self._fused_in_proj_weight
            if hasattr(torch.ops.vllm, "qwen4_exp_low_latency_gemm"):
                out = torch.ops.vllm.qwen4_exp_low_latency_gemm(hidden_states, w)
            else:
                out = torch.nn.functional.linear(hidden_states, w)
            return out.split(list(self._fused_in_proj_sizes), dim=-1)
    ...
    def forward_cuda(
        ...
        # ============================================================
        mixed_qkvz, ba = self._project_in(hidden_states)
