    ...
    def load_weights(
        ...
        for name, loaded_weight in weights:
            ...
            if param is None and name == "bias":
                continue
            if param is self:
                # saturn (2026-09-05): upstream silently falls back to the
                # module itself and dies later on `.data`; name the culprit.
                raise AttributeError(
                    f"{type(self).__name__} '{self.prefix}' has no parameter "
                    f"'{name}' (shard {shard_id}, loaded shape "
                    f"{tuple(loaded_weight.shape)} {loaded_weight.dtype}); "
                    f"method {type(self.quant_method).__name__}, params "
