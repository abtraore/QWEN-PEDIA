...
class ModelOptMixedPrecisionConfig(ModelOptQuantConfigBase):
    ...
    def get_quant_method(
        ...
        if isinstance(layer, RoutedExperts):
            ...
            if quant_algo == "FP8_BLOCK_SCALES":
                # saturn (2026-09-05): nvidia/Qwen3.8-Flash-Next-NVFP4 stores
                # the MTP routed experts as 128x128 block-scaled FP8
                # (`weight_scale_inv` [out/128, in/128], DeepSeek layout).
                # ModelOpt's own FP8 MoE method is per-tensor; vLLM's block
                # FP8 MoE method (fp8.py) is the matching loader and kernel.
                from vllm.model_executor.layers.quantization.fp8 import (
                    Fp8Config,
                    Fp8MoEMethod,
                )

                group = self._resolve_group_size(prefix) or 128
                block_cfg = Fp8Config(
                    is_checkpoint_fp8_serialized=True,
    ...
    def _resolve_group_size(self, prefix: str) -> int | None:
        """group_size of the quantized_layers entry that matched ``prefix``."""
        for candidate in self._quantized_layer_prefix_candidates(prefix):
            info = self.quantized_layers.get(candidate)
            if info is not None:
                return info.get("group_size")
            prefix_dot = candidate + "."
            for key, info in self.quantized_layers.items():
                if key.startswith(prefix_dot):
                    return info.get("group_size")
