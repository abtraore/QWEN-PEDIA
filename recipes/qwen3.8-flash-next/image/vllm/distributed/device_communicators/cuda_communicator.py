...
class CudaCommunicator(DeviceCommunicatorBase):
    ...
    def all_gather(self, input_: torch.Tensor, dim: int = -1) -> torch.Tensor:
        ...
        # saturn (2026-09-05): route small all-gathers through the custom
        # one-shot IPC all-gather kernel that ships next to custom all-reduce
        # (never reached from here upstream). On 4x PCIe 5090s the target's
        # 2 MB logits gather cost ~0.5 ms/step on NCCL; the P2P kernel reads
        # peers' buffers directly. Opt in with VLLM_CUSTOM_ALL_GATHER=1;
        # anything the kernel refuses (size > its buffer, dtype) falls
        # through to NCCL unchanged.
        ca_comm = self.ca_comm
        if (
