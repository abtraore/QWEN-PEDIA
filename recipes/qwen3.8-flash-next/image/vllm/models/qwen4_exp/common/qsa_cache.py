...
class QSAForwardMetadata(AttentionMetadata):
    ...
    storage_block_size: int
    compress_ratio: int
    # for in-place refresh between MTP draft steps (fused multi-step draft):
    # the group's common slot mapping (recomputed in place by the speculator)
    # and the mapped-token count (constant across draft steps)
...
class QSAMetadataBuilder(AttentionMetadataBuilder[QSAForwardMetadata]):
    ...
    def __init__(
        ...
        # Fused multi-step draft decode: refresh position-dependent metadata in
        # place between MTP draft steps instead of rebuilding it (saturn
        # 2026-09-02; QSA_FUSED_DRAFT=0 disables). Triton path only.
        self.supports_draft_decode_metadata_update = (
            HAS_TRITON and os.environ.get("QSA_FUSED_DRAFT", "1") != "0"
        )
    ...
    def update_draft_decode_metadata(self, metadata: QSAForwardMetadata) -> None:
        """Refresh slot_mapping / logical_positions / token_to_req / k_work in
        place after the speculator advanced positions and seq_lens (live views)
        and recomputed the common slot mapping. Same kernel as build(); the
        metadata fields are views into this builder's buffers."""
        num_tokens = metadata.num_actual_tokens
        if num_tokens == 0 or metadata.common_slot_mapping is None:
            return
        build_k_work = not self.is_circular_buffer and self.compress_ratio != 1

        class _Shim:  # the subset of CommonAttentionMetadata the kernel reads
            pass
        shim = _Shim()
        shim.num_actual_tokens = num_tokens
        shim.query_start_loc_cpu = torch.tensor([metadata.num_mapped_tokens])
        shim.query_start_loc = metadata.query_start_loc
        shim.seq_lens = metadata.seq_lens
        shim.slot_mapping = metadata.common_slot_mapping
        shim.block_table_tensor = metadata.block_table
        build_qsa_metadata(
            shim,
            self.token_to_req_buffer,
            self.logical_positions_buffer,
...
class QSAStateBackend(AttentionBackend):
    ...
    # The side caches pin their own bf16 dtypes (cache_dtype_str is ignored
    # in get_kv_cache_spec below); accepting the fp8 strings only lets the
    # engine-level validation pass when the MAIN cache runs fp8.
