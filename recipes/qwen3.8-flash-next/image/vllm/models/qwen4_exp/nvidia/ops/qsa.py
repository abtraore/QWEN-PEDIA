...
def _qsa_sparse_paged_gqa_splitk_kernel(
    ...
    if IS_FP8:
        # Per-tensor dequant scales, read from device buffers so cudagraph
    ...
    for tile in range(split_id, tile_end, NUM_SPLITS):
        ...
        if IS_FP8:
            # Fold the per-tensor K dequant scale into the logit scaling.
            scores *= k_scale
    ...
    if IS_FP8:
        # Fold the per-tensor V dequant scale into the finished output.
...
def warmup_qsa_sparse_paged_attention(
    ...
    *,
    ...
    # saturn fp8-KV port: the fp8 cache is allocated as uint8 and viewed as
    # e4m3 at call time; warm the kernel with the dtype it will actually see.
    ...
    for block_n, warps, num_tiles, num_splits in sorted(profiles):
        ...
        _qsa_sparse_paged_gqa_splitk_kernel.warmup(
            ...
            k_cache_ptr,
            v_cache_ptr,
            # saturn fp8-KV port: per-tensor scale pointers (dummies when bf16).
            scale_ptr,
            scale_ptr,
            indices_ptr,
            block_table_ptr,
