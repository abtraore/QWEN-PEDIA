...
class Qwen4ExpMTP(nn.Module, SupportsPP, Qwen4ExpMixtureOfExperts):
    ...
    def get_top_tokens(
        ...
        saturn (2026-09-05): the profiled decode step spent ~2 ms of its
        13 ms in NCCL all-gathers, three of the four large ones being the
        draft steps' full-vocab logits gather (4 x 248,320 bf16 per step).
        Argmax over the local shard + an all-gather of (value, id) pairs
        selects the same token; the proposer uses this when
        ``use_local_argmax_reduction`` is set in the speculative config.
        Name fixed by the protocol the proposer probes for.
        """
        del spec_step_idx
