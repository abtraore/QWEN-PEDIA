...
class MmapPleTable:
    ...
    def __init__(
        ...
        for idx, (path, offset, rows) in shards.items():
            ...
            # saturn (2026-09-04): rows are 160 B and access is random, so the kernel's
            # default readahead (128 KB per fault on /tank) multiplies every cold-row read.
            # MADV_RANDOM measured 3x on cold gathers (40 rows: 11.9 -> 3.8 ms).
            if os.getenv("VLLM_PLE_MMAP_MADV_RANDOM", "1") != "0":
                try:
                    import mmap as _mmap

                    self.mm[idx]._mmap.madvise(_mmap.MADV_RANDOM)
                except Exception as exc:  # noqa: BLE001
                    logger.warning_once("PLE mmap: MADV_RANDOM failed (%s)", type(exc).__name__)
    ...
    def mlock_all(self) -> int:
        """Pin every shard mapping in RAM (saturn, 2026-09-04).

        The host's kswapd reclaims file pages under THP/compaction pressure
        even with tens of GB free, which evicts the prewarmed table between
        boots and puts cold-row reads back on the decode path. mlock makes
        the pages unevictable. Needs RLIMIT_MEMLOCK >= table size (compose:
        ulimits memlock -1). Returns bytes locked; logs and returns 0 on
        failure (the mmap path keeps working, just evictable).
        """
        locked = 0
        for mm in self.mm:
            if mm is None:
                continue
            nbytes = self._mlock_one(mm)
            if nbytes < 0:
                logger.warning(
                    "PLE mmap: mlock failed after %.2f GiB; table stays evictable",
                    locked / (1 << 30),
                )
                return locked
            locked += nbytes
        logger.info("PLE mmap: mlocked %.2f GiB of table", locked / (1 << 30))
        return locked

    @staticmethod
    def _mlock_one(mm) -> int:
        """mlock one shard mapping; returns bytes locked, -1 on failure."""
        import ctypes

        libc = ctypes.CDLL(None, use_errno=True)
        libc.mlock.argtypes = [ctypes.c_void_p, ctypes.c_size_t]
        nbytes = mm.shape[0] * mm.shape[1]
        if libc.mlock(ctypes.c_void_p(mm.ctypes.data), ctypes.c_size_t(nbytes)) != 0:
            err = ctypes.get_errno()
            logger.warning("PLE mmap: mlock errno %d: %s", err, os.strerror(err))
            return -1
        return nbytes

    def prewarm(self, max_bytes: int, lock: bool = False) -> int:
        """Stream up to ``max_bytes`` of the table into the page cache.

        Args:
            max_bytes: byte budget; a non-positive value skips prewarm.
            lock: saturn (2026-09-05): mlock each shard right after it is
                streamed, so kswapd cannot evict it before a later
                ``mlock_all`` pass (which then re-faulted ~half the table
                from disk and doubled the boot).
        ...
        for mm in self.mm:
            ...
            if lock and not lock_failed and pos >= end:
                if self._mlock_one(mm) < 0:
                    lock_failed = True
        return read_total
...
class MmapNgramEmbedding(nn.Module):
    ...
    def __init__(self, num_embeddings: int, embedding_dim: int) -> None:
        ...
        # saturn (2026-09-04): GPU-resident row cache. N-gram ids repeat
        # heavily inside a session, so rows already fetched are kept on the
        # device and only the misses take the host mmap round trip. Sized in
        # rows by VLLM_PLE_MMAP_ROW_CACHE (0 = off); FIFO ring eviction; the
        # id->slot map lives on the host where the ids already are after the
        # D2H copy. Lazily allocated on the first real gather.
        self._row_cache_rows = max(0, int(os.getenv("VLLM_PLE_MMAP_ROW_CACHE", "0")))
        self._row_cache: torch.Tensor | None = None
        self._row_cache_ids: np.ndarray | None = None
        self._row_cache_map: dict[int, int] = {}
        self._row_cache_next = 0
        self._cache_hits_since_log = 0
        self._cache_lookups_since_log = 0

    def _row_cache_gather(
        self, ids_np: np.ndarray, table: "MmapPleTable", destination: torch.Tensor
    ) -> tuple[float, int, int]:
        """Serve ``ids_np`` from the device row cache, fetching misses.

        Returns ``(gather_ms, hits, lookups)``; ``gather_ms`` covers the host
        table gather of the misses only.
        """
        device = destination.device
        if self._row_cache is None:
            self._row_cache = torch.empty(
                (self._row_cache_rows, table.row_bytes), dtype=torch.uint8, device=device
            )
            self._row_cache_ids = np.full(self._row_cache_rows, -1, dtype=np.int64)
        uniq, inverse = np.unique(ids_np, return_inverse=True)
        cmap = self._row_cache_map
        slots = np.empty(uniq.size, dtype=np.int64)
        miss_pos: list[int] = []
        ...
        if miss_pos:
            ...
            rows = table.gather(miss_ids)  # uint8 [M, row_bytes]
            gather_ms = (time.monotonic() - gather_t) * 1000.0
            m = miss_ids.size
            cap = self._row_cache_rows
            start = self._row_cache_next
            new_slots = (np.arange(m, dtype=np.int64) + start) % cap
            self._row_cache_next = int((start + m) % cap)
            # Evict whatever those slots held.
            old = self._row_cache_ids[new_slots]
            for oid in old[old >= 0].tolist():
                cmap.pop(oid, None)
            self._row_cache_ids[new_slots] = miss_ids
            for rid, sl in zip(miss_ids.tolist(), new_slots.tolist()):
                cmap[rid] = sl
            slots[miss_pos] = new_slots
            src = torch.from_numpy(rows)
            if self.pinned:
                src, engaged = _stage_pinned(src)
                self.pinned = engaged
            idx = torch.from_numpy(new_slots).to(device, non_blocking=True)
            self._row_cache.index_copy_(0, idx, src.to(device, non_blocking=True))
        # Assemble the destination from cache slots, in input order.
        order = torch.from_numpy(slots[inverse]).to(device, non_blocking=True)
        gathered = self._row_cache.index_select(0, order)
        destination.copy_(gathered.view(table.torch_dtype).reshape(destination.shape))
        return gather_ms, uniq.size - len(miss_pos), uniq.size
    ...
    def gather_into(self, ids: torch.Tensor, destination: torch.Tensor) -> None:
        ...
        if self._row_cache_rows > 0 and ids.device.type == "cuda":
            if table.row_bytes != self.embedding_dim * table.itemsize:
                raise ValueError(
                    f"PLE mmap: table row_bytes={table.row_bytes} does not "
                    f"match embedding_dim={self.embedding_dim} * "
                    f"itemsize={table.itemsize}"
                )
            gather_ms, hits, lookups = self._row_cache_gather(ids_np, table, destination)
            self._cache_hits_since_log += hits
            self._cache_lookups_since_log += lookups
            self._record_input_prep_timing(
                ids_np.size, ids_d2h_wait_ms, gather_ms, 0.0, pinned=self.pinned
            )
            return
    ...
    def _record_input_prep_timing(
        ...
        *,
        ...
        logger.info(
            ...
            "p99_h2d_call_ms=%.2f pinned=%d/%d row_cache_hit=%.3f (%d/%d)",
            ...
            n,
            (self._cache_hits_since_log / self._cache_lookups_since_log)
            if self._cache_lookups_since_log
            else 0.0,
            self._cache_hits_since_log,
            self._cache_lookups_since_log,
        )
        self._cache_hits_since_log = 0
        self._cache_lookups_since_log = 0
...
def _attach_table(
    ...
    try:
        ...
        if envs.VLLM_PLE_MMAP_PREWARM:
            ...
            lock_during_prewarm = os.getenv("VLLM_PLE_MMAP_MLOCK", "0") == "1"
            read = table.prewarm(bound, lock=lock_during_prewarm)
            logger.info(
            ...
            if os.getenv("VLLM_PLE_MMAP_MLOCK", "0") == "1":
                table.mlock_all()
