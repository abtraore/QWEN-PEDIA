...
# saturn (2026-09-05): consumer Blackwell (RTX 5090, sm120) has no plan table
# upstream, so every bf16 dense projection (the modelopt excludes: GDN and QSA
# projections, hyper-connection mixers, router, shared expert, MTP head) runs
# through cuBLAS, which picks an sm80 WMMA kernel at ~9 us per launch. The
# profiled TP=4 decode step spends 4.7 ms of its 13 ms in 528 of those launches
# against a ~1.2 ms bandwidth floor. Opt in with
# VLLM_QWEN4_EXP_SKINNY_GEMM_SM120=1; the plan starts from the sm103 table
# (same generation, no tcgen05/TMA dependence in the kernel) and can be
# replaced by a measured JSON via VLLM_QWEN4_EXP_SKINNY_GEMM_PLANS_FILE (see
# tools/qwen4_exp_skinny_gemm_tune.py for the sweep that writes it).
def _load_plans_file(path: str) -> dict[tuple[int, int], dict[int, SkinnyGemmConfig]]:
    import json

    with open(path) as f:
        raw = json.load(f)
    plans: dict[tuple[int, int], dict[int, SkinnyGemmConfig]] = {}
    for shape, by_m in raw.items():
        n, k = (int(v) for v in shape.split("x"))
        plans[(n, k)] = {
            int(m): SkinnyGemmConfig(**cfg) for m, cfg in by_m.items()
        }
    return plans


_SM120_PLANS_CACHE: dict[tuple[int, int], dict[int, SkinnyGemmConfig]] | None = None


def _sm120_plans() -> dict[tuple[int, int], dict[int, SkinnyGemmConfig]]:
    global _SM120_PLANS_CACHE
    if _SM120_PLANS_CACHE is not None:
        return _SM120_PLANS_CACHE
    import os

    if os.getenv("VLLM_QWEN4_EXP_SKINNY_GEMM_SM120", "0") != "1":
        _SM120_PLANS_CACHE = {}
        return _SM120_PLANS_CACHE
    path = os.getenv("VLLM_QWEN4_EXP_SKINNY_GEMM_PLANS_FILE", "")
    if path:
        _SM120_PLANS_CACHE = _load_plans_file(path)
        logger.info(
            "qwen4_exp skinny GEMM (sm120): loaded %d shapes from %s",
            len(_SM120_PLANS_CACHE),
            path,
        )
    else:
        _SM120_PLANS_CACHE = QWEN4_EXP_GEMM_PLANS
        logger.info(
            "qwen4_exp skinny GEMM (sm120): using the sm103 plan table "
            "(untuned; set VLLM_QWEN4_EXP_SKINNY_GEMM_PLANS_FILE for measured plans)"
        )
    return _SM120_PLANS_CACHE
...
def maybe_fuse_gdn_in_proj(module: nn.Module) -> None:
    """saturn (2026-09-05): fuse each GDN layer's qkvz+ba input projections
    into one GEMM when VLLM_GDN_FUSE_IN_PROJ=1. Call after weight loading.
    The fused shape (e.g. 4120x2560 at TP=4) needs its own skinny plan entry
    to benefit from the low-latency kernel; without one it runs on cuBLAS."""
    import os

    if os.getenv("VLLM_GDN_FUSE_IN_PROJ", "0") != "1":
        return
    n = 0
    seen = 0
    reason = None
    for child in module.modules():
        fuse = getattr(child, "fuse_in_proj", None)
        ...
        if fuse():
            ...
            reason = getattr(child, "_fuse_in_proj_reason", "unknown")
    logger.info(
        "qwen4_exp: fused qkvz+ba input projection on %d of %d GDN layers%s",
        n,
        seen,
        "" if reason is None else f" (first refusal: {reason})",
    )
