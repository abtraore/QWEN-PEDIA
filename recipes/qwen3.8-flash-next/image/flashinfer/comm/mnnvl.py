...
class SymmDeviceMemory:
    ...
    def _get_allocation_prop(self, buf_size: int):
        ...
        # Rig patch: GPUDirect RDMA is unsupported on consumer GPUs (GeForce
        # RTX 5090 reports CU_DEVICE_ATTRIBUTE_GPU_DIRECT_RDMA_SUPPORTED=0) and
        # cuMemCreate then fails with CUDA_ERROR_INVALID_DEVICE when the flag is
        # requested. Ask for it only when the device supports it.
        _rdma_supported = checkCudaErrors(
            cuda.cuDeviceGetAttribute(
                cuda.CUdevice_attribute.CU_DEVICE_ATTRIBUTE_GPU_DIRECT_RDMA_SUPPORTED,
                self.device_idx,
            )
        )
        allocation_prop.allocFlags.gpuDirectRDMACapable = 1 if _rdma_supported else 0
