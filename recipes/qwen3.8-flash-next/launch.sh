#!/bin/bash
# Qwen3.8-Flash-Next (nvidia NVFP4) on 4x RTX 5090, build r10. Measured 2026-09-12 (NOTES.md):
# 106 engine steps/s, 319-331 tok/s decode on code, prefill ~7,800 tok/s at 36K and 100K,
# KV pool 736,359 tokens at the 393,216-token YaRN context, boot ~10 min.
# Build first:  docker build -t qwen-pedia/fnext-vllm:r10 .
# Needs: 4x 32 GB sm120 cards, ~135 GB disk for the checkpoint, 125 GB host RAM (the 48 GB
# n-gram table lives in pinned host memory, read by the GPU over UVA), ulimit memlock unlimited.
set -e
